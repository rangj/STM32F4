...
#ifdef sEE_M24C16               //24c16 have 8 block,every block have 256bytes. select which block depend on I2C_Send7bitAddress
  block_select = (ReadAddr & 0x0700) >> 7;
  I2C_Send7bitAddress(sEE_I2C, (sEEAddress | block_select), I2C_Direction_Transmitter);
#elif define sEE_M24C64_32
...
#ifdef sEE_M24C16               //24c16 have 8 block,every block have 256bytes. select which block depend on I2C_Send7bitAddress
  block_select = (ReadAddr & 0x0700) >> 7;
  I2C_Send7bitAddress(sEE_I2C, (sEEAddress | block_select), I2C_Direction_Receiver);
#elif define sEE_M24C64_32
...
uint32_t sEE_WritePage(uint8_t* pBuffer, uint16_t WriteAddr, uint8_t* NumByteToWrite)
{
  ...
  sEETimeout = sEE_FLAG_TIMEOUT;
#ifdef sEE_M24C16               //24c16 have 8 block,every block have 256bytes. select which block depend on I2C_Send7bitAddress
  block_select = (WriteAddr & 0x0700) >> 7;
  I2C_Send7bitAddress(sEE_I2C, (sEEAddress | block_select), I2C_Direction_Transmitter);
#elif define sEE_M24C64_32
  I2C_Send7bitAddress(sEE_I2C, sEEAddress, I2C_Direction_Transmitter);
#endif
...
uint32_t sEE_TIMEOUT_UserCallback(void)
{
  /* Block communication and all processes */
    //GPIO_WriteBit(GPIOE, GPIO_Pin_2, !GPIO_ReadOutputDataBit(GPIOE, GPIO_Pin_2));//fulk,Íü¼Ç×¢ÊÍµô(2013.04.27)
    return 0;
    /*
  while (1)
  {
